...
Fantasy golf league dashboard for the Sub-Par Golf Association (me and my friends). Tracks live and historical scores across the four majors, computes per-hole points and bonuses for each manager's roster, and renders a sortable leaderboard.
...
| File | Purpose |
|------|---------|
| `data.js` | Set `ACTIVE_TOURNAMENT`, define manager rosters per major, configure `POINTS_CONFIG`, store `DRAFT_LOG` |
| `app.js` | ESPN API fetch, score parsing, leaderboard computation, and all UI rendering |
| `index.html` | Static shell — tournament banner, sortable leaderboard table, points guide, navigation tabs |
